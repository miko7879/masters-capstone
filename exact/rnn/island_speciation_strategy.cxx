...
IslandSpeciationStrategy::IslandSpeciationStrategy(
    ...
    //FALA: Calculate the starting rate
    std_rate = 1.0/number_of_islands;
    Log::info("<FALA> Island standard probability: %f\n", std_rate);
    
    //FALA: Learning stages
    genomes_per_stage = number_of_islands*max_island_size*8;
    ...
    for (int32_t i = 0; i < number_of_islands; i++) {
        ...
        //FALA: Island FALA variables
        island_rates.push_back(std_rate);
        island_reinforcement.push_back(0);
    }
    
    //FALA: Island FALA rates
    island_lr = 0.007;
    min_rate = std_rate;
    degr_rate = std_rate;
    Log::info("<FALA> Island FALA LR: %f\n", island_lr);
...
int32_t IslandSpeciationStrategy::insert_genome(RNN_Genome* genome) {
    ...
    //Reset island reinforcement
    for (int32_t i = 0; i < number_of_islands; i++) {
        island_reinforcement[i] = 0;
    }
    
    //FALA: Island reinforcement based on performance of genome
    if(curr_stage > 0){
        if(insert_position != -1){
            island_reinforcement[island] += global_best_genome->get_fitness()/genome->get_fitness();
        } else if (island_rates[island] > degr_rate){
            island_reinforcement[island] -= 0.5*island_rates[island]/degr_rate;
        }
    }
    ...
    if (global_best_genome == NULL) {
        ...
        global_best_genome = genome->copy();
        new_global_best = true;
        //FALA: Keep track of best island
        if(island != best_island){
            best_island = island;
        }
    }

    Log::info("<FALA> Best island: %d\n", best_island);
    
    inserted_genomes++;
    

    
    //FALA: If stage 0 has been passed, adjust rates
    if(curr_stage > 0){
        //FALA: Small reinforcement based on best island
        if(island_rates[best_island] < degr_rate){
            island_reinforcement[best_island] += 0.5*island_rates[best_island]/degr_rate/10;
        }
        //FALA: Update island rates using FALA
        double norm_to = 1.0;
        double norm_factor = 1.0;
        std::vector<bool> normalize(NUM_RATES, true);
        //FALA: Check if any rates are below min threshold
        for(int i = 0; i < number_of_islands; i++){
            island_reinforcement[i] = island_reinforcement[i]*island_lr;
            Log::info("<FALA> Island Reinforcement [%d] = %f\n", i, island_reinforcement[i]);
            norm_factor += island_reinforcement[i];
            island_rates[i] += island_reinforcement[i];
            if(island_rates[i] < min_rate){
                norm_factor -= island_rates[i];
                normalize[i] = false;
                island_rates[i] = min_rate;
        ...
        for(int i = 0; i < number_of_islands; i++){
            ...
            Log::info("<FALA> Island Rates [%d] = %f\n", i, island_rates[i]);
        }
    }
    ...
    if(genomes_in_stage == genomes_per_stage){
        genomes_in_stage = 0;
        min_rate -= std_rate/4.0/(num_stages - 1);
        degr_rate += std_rate/4.0/(num_stages - 1);
        curr_stage++;
        Log::info("<FALA> Stage %d reached, minimum island rate: %f, degradation start rate: %f\n", curr_stage, min_rate, degr_rate);
        for(int i = 0; i < number_of_islands; i++){
            island_rates[i] = std_rate;
        }
    } else{
        genomes_in_stage++;
    }
    ...
    if(new_global_best){
        return 0; //If global best, return 0
    } else if (insert_position == -1){
        return -1; //If not inserted, return -1
    } else {
        return insert_position + 1; //Otherwise, return the insert position + 1
    }
...
int IslandSpeciationStrategy::get_generation_island(uniform_real_distribution<double> &rng_0_1, minstd_rand0 &generator){
    ...
    //FALA: If stage 0 has been passed, draw current_island from evolving distribution
    else {
        double r = rng_0_1(generator);
        double cum_sum = 0;
        for(int i = 0; i < number_of_islands; i++){
...
int IslandSpeciationStrategy::get_genomes_in_stage(){
    return genomes_in_stage;
}
